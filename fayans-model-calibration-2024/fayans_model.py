...
loads the rescaled model that predicts the residuals for a 13-dim design x, and
defines the following public functions that are ready for usage by ParMOO:
 - the ParMOO compatible sim_func ``fayans_model_sim(x)``
 - the ParMOO compatible sim_func ``fayans_blackbox_model_sim(x)``
 - the ParMOO compatible obj_func ``binding_energy_obj_func(x, s)``
 - the ParMOO compatible obj_grad ``binding_energy_obj_grad(x, s)``
 - the ParMOO compatible obj_func ``std_radii_obj_func(x, s)``
 - the ParMOO compatible obj_grad ``std_radii_obj_grad(x, s)``
 - the ParMOO compatible obj_func ``other_quantities_obj_func(x, s)``
 - the ParMOO compatible obj_grad ``other_quantities_obj_grad(x, s)``
 - the ParMOO compatible con_func ``binding_energy_con_func(x, s)``
 - the ParMOO compatible con_grad ``binding_energy_con_grad(x, s)``
 - the ParMOO compatible con_func ``std_radii_con_func(x, s)``
 - the ParMOO compatible con_grad ``std_radii_con_grad(x, s)``
 - the ParMOO compatible con_func ``other_quantities_con_func(x, s)``
 - the ParMOO compatible con_grad ``other_quantities_con_grad(x, s)``
...
# Define the global constant parmoo input keys for later use
...
def fayans_model_sim(x):
    """ Predict the fit residuals using a pre-trained model.

    Args:
        x (dict): contains 13 float-valued keys,
...
def fayans_blackbox_model_sim(x):
    ...
    s = {"residuals": fayans_model_sim(x)}
    # Calculate the 3 outputs using the objectives defined below
    s_out = np.zeros(3)
    s_out[0] = binding_energy_obj_func(x, s)
    s_out[1] = charge_radius_obj_func(x, s) + diffraction_radius_obj_func(x, s)
    s_out[2] = (surface_thickness_obj_func(x, s)     +
                neutron_single_level_obj_func(x, s)  +
                proton_single_level_obj_func(x, s)   +
                isotopic_shifts_radii_obj_func(x, s) +
                neutron_pairing_gap_obj_func(x, s)   +
                proton_pairing_gap_obj_func(x, s))
    return s_out

def binding_energy_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    binding energy class based on the output of the ``fayans_model_sim``.

    Args:
        x (dict): contains 13 float-valued keys, defined in DES_NAMES

        s (dict): contains a single key "residuals" with 198 float-valued
            residuals, as output by the ``fayans_model_sim`` function
    ...
    return jnp.dot(s["residuals"][0:63], s["residuals"][0:63])

def binding_energy_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``binding_energy_obj_func``.
    ...
    Returns:
        dict, dict: the gradients with respect to x and s, respectively

    """

    dx = {}
    ds = {"residuals": jnp.zeros(198)}
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[0:63].set(2.0 * s["residuals"][0:63])
    return dx, ds

def charge_radius_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    charge radius class based on the output of the ``fayans_model_sim``.
    ...
    return jnp.dot(s["residuals"][117:169], s["residuals"][117:169])

def charge_radius_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``charge_radius_obj_func``.
    ...
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[117:169].set(2.0 * s["residuals"][117:169])
    return dx, ds

def diffraction_radius_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    charge radius class based on the output of the ``fayans_model_sim``.
    ...
    return jnp.dot(s["residuals"][63:91], s["residuals"][63:91])

def diffraction_radius_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``diffraction_radius_obj_func``.
    ...
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[63:91].set(2.0 * s["residuals"][63:91])
    return dx, ds

def std_radii_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    standard radii class based on the output of the ``fayans_model_sim``.
    ...
    return charge_radius_obj_func(x, s) + diffraction_radius_obj_func(x, s)

def std_radii_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``std_radii_obj_func``.
    ...
    dx, ds = charge_radius_obj_grad(x, s)
    _, ds2 = diffraction_radius_obj_grad(x, s)
    ds["residuals"] += ds2["residuals"]
    return dx, ds

def surface_thickness_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    surface thickness class based on the output of the ``fayans_model_sim``.

    Args:
        x (dict): contains 13 float-valued keys, defined in DES_NAMES

        s (dict): contains a single field "residuals" with 198 float-valued
            residuals, as output by the ``fayans_model_sim`` function
    ...
    return jnp.dot(s["residuals"][91:117], s["residuals"][91:117])

def surface_thickness_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``surface_thickness_obj_func``.
    ...
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[91:117].set(2.0 * s["residuals"][91:117])
    return dx, ds

def neutron_single_level_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    neutron single level class based on the output of the ``fayans_model_sim``.
    ...
    return jnp.dot(s["residuals"][174:179], s["residuals"][174:179])

def neutron_single_level_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``neutron_single_level_obj_func``.
    ...
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[174:179].set(2.0 * s["residuals"][174:179])
    return dx, ds

def proton_single_level_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    proton single level class based on the output of the ``fayans_model_sim``.
    ...
    return jnp.dot(s["residuals"][169:174], s["residuals"][169:174])

def proton_single_level_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``proton_single_level_obj_func``.
    ...
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[169:174].set(2.0 * s["residuals"][169:174])
    return dx, ds

def isotopic_shifts_radii_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    isotopic shifts radii class based on the output of ``fayans_model_sim``.
    ...
    return jnp.dot(s["residuals"][179:182], s["residuals"][179:182])

def isotopic_shifts_radii_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``isotopic_shifts_radii_obj_func``.
    ...
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[179:182].set(2.0 * s["residuals"][179:182])
    return dx, ds

def neutron_pairing_gap_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    neutron pairing gap class based on the output of the ``fayans_model_sim``.
    ...
    return jnp.dot(s["residuals"][182:187], s["residuals"][182:187])

def neutron_pairing_gap_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``neutron_pairing_gap_obj_func``.
    ...
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[182:187].set(2.0 * s["residuals"][182:187])
    return dx, ds

def proton_pairing_gap_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals in the
    proton pairing gap class based on the output of the ``fayans_model_sim``.
    ...
    return jnp.dot(s["residuals"][187:198], s["residuals"][187:198])

def proton_pairing_gap_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``proton_pairing_gap_obj_func``.
    ...
    for name in DES_NAMES:
        dx[name] = 0.0
    ds["residuals"].at[187:198].set(2.0 * s["residuals"][187:198])
    return dx, ds

def other_quantities_obj_func(x, s):
    """ A ParMOO objective that calculates the sum-of-squared residuals across
    all nonstandard observation classes (surface thickness, neutron single
    level, proton single level, isotopic shifts radii, neutron pairing gap, and
    proton pairing gap) based on the output of the ``fayans_model_sim``.
    ...
    return (surface_thickness_obj_func(x, s)     +
            neutron_single_level_obj_func(x, s)  +
            proton_single_level_obj_func(x, s)   +
            isotopic_shifts_radii_obj_func(x, s) +
            neutron_pairing_gap_obj_func(x, s)   +
...
def other_quantities_obj_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``other_quantities_obj_func``.
    ...
    dx, ds = surface_thickness_obj_grad(x, s)
    _, ds2 = neutron_single_level_obj_grad(x, s)
    ds["residuals"] += ds2["residuals"]
    _, ds3 = proton_single_level_obj_grad(x, s)
    ds["residuals"] += ds3["residuals"]
    _, ds4 = isotopic_shifts_radii_obj_grad(x, s)
    ds["residuals"] += ds4["residuals"]
    _, ds5 = neutron_pairing_gap_obj_grad(x, s)
    ds["residuals"] += ds5["residuals"]
    _, ds6 = proton_pairing_gap_obj_grad(x, s)
    ds["residuals"] += ds6["residuals"]
    return dx, ds

def binding_energy_con_func(x, s):
    ...
    return binding_energy_obj_func(x, s) - 804

def binding_energy_con_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``binding_energy_con_func``.
    ...
    return binding_energy_obj_grad(x, s)

def std_radii_con_func(x, s):
    ...
    return std_radii_obj_func(x, s) - 2090

def std_radii_con_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``std_radii_con_func``.
    ...
    return std_radii_obj_grad(x, s)

def other_quantities_con_func(x, s):
    ...
    Args:
        ...
        s (dict): contains a single key "residuals" with 198 float-valued keys
            standard residuals, as output by the ``fayans_model_sim`` function
    ...
    return other_quantities_obj_func(x, s) - 613

def other_quantities_con_grad(x, s):
    """ A ParMOO gradient function that calculates the the gradient of the
    ``other_quantities_con_func``.
    ...
    return other_quantities_obj_grad(x, s)
...
if __name__ == "__main__":
    ...
    ae = np.abs(__rescale__(fayans_model_sim(x1)) -
